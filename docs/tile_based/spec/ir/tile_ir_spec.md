# Tile IR Specification (TileDesc 기반 실행 DAG / Tile Ops)

본 문서는 Tile IR 개념을 **하나의 기준 문서로 통합**한 스펙이다.
...
- 메인 스펙(`docs/spec/ir/npu_ir_spec.md`)은 **Layer-level IR → TileGraph → CMDQ** 흐름을 기준으로 한다.
- 본 Tile IR은 그 흐름을 대체하지 않으며, **TileGraph 이후(tiling/memory planning 이후)** 단계에서
  스케줄러/시뮬레이터 인터페이스를 명확히 하기 위한 **선택적(대안적) 표현**이다.
- 따라서 메인 스펙의 단일 소스 오브 트루스는 유지되며, Tile IR 채택 여부는 구현/실험 범위에 따라 결정된다.

---

## 1. 문서 목적

Tile IR은 Tile 기반 NPU 시뮬레이터에서 다음을 고정한다.

- 실행 단위: Tile (atomic, 스케줄링/분석 단위)
- 실행 구조: Tile Dependency Graph (TDG) 기반의 **명시적 DAG**
- 실행 의미론: DRAM ↔ SPM(=Global SRAM) 이동은 명시적이며, 동기화는 명시적
- 인터페이스: IR → Scheduler/Simulator로 전달되는 최소 정보 집합
...
Tile IR은 `../architecture/`의 메모리/데이터플로우/라이프사이클 규칙을 전제로 한다.
...
## 4. 실행 모델(Temporal Semantics)

Tile IR은 정적 스케줄링 기반 실행을 전제로 한다.

- (권고) 코어별 실행 스트림은 정적으로 결정된다. (`npu_ir_lowering_and_execution.md` 참고)
...
- SPM(=Global SRAM): 온칩 소프트웨어 관리 메모리, Tile payload의 기본 상주 공간
...
## 8. Tile Ops (Core Op Schema)

아래는 Tile IR이 제공해야 하는 최소 op schema 예시이다.

### 8.1 Compute / Vector Ops
...
동기화의 정확한 모델(예: tag/wait)은 `npu_ir_core_spec.md` 및 로어링 스펙에서 구체화한다.
