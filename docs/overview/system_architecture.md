...
메모리 transaction이 줄어드는 구조를 정밀하게 반영한다
...
모든 엔진은 한 cycle 단위 업데이트

실 칩 DRAM controller의 미세한 timing은 모델링하지 않음(대역폭/버스트 기반만 반영)
